...
def init_mutation(project):
    ...
    recovered_path = s2e_dir + "/projects/" + project + "/s2e-out/recovered.ll"
...
def clone_recovered(project):
    ...
    if not os.path.isfile(s2e_dir + "/projects/"+str(project)+ "/s2e-out/recovered.ll"):
        print("Incorrect project, or project hasn't been recovered")
        quit()

    copy = s2e_dir + "/projects/" + project + "/s2e-out/recovered.ll"
    original = s2e_dir + "/projects/" + project + "/s2e-out/original_recovered.ll"
...
def get_section_table(project):
    ...
    binary_path = s2e_dir + "/projects/" + project + "/s2e-out/binary"
...
def make_mutation_folder(project):
    ...
    mutation_folder = s2e_dir + "/projects/" + project + "/s2e-out/mutations"
...
def ro_txt_addresses(project):
    ...
    ro = []
    txt = []
    sections = s2e_dir + "/projects/" + project + "/s2e-out/sections"
...
def address_could_be_string(project, address):
    ...
    with open(s2e_dir + "/projects/" + project + "/binary", 'br') as f:
        byte = f.read()[offset:]
...
def reset_recovered(project):
    ...
    Return: 0 if success
    """
    recovered = s2e_dir + "/projects/" + project + "/s2e-out/recovered.ll"
    original = s2e_dir + "/projects/" + project + "/s2e-out/original_recovered.ll"
...
def save_mutation(project, infos):
    ...
    recompile(project)
    mutation_folder = s2e_dir + "/projects/" + project + "/s2e-out/mutations"
    mutation = s2e_dir + "/projects/" + project + "/s2e-out/custom_recovered"
